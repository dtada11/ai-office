...
> [!WARNING]
> **현재 릴리스 설치 보류 권장 (2026-07-16).** 자체 코드 점검에서 데이터 유실·크래시급 결함이 확인되어 수정 중입니다. 특히 훅 설치 과정이 손상된 `~/.claude/settings.json`을 덮어써 사용자의 다른 Claude Code 설정이 사라질 수 있는 경로가 있습니다. **수정본을 릴리스하기 전까지는 설치·실행하지 마세요.** 코드 열람·리뷰 용도로만 봐 주세요.
