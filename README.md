...
> **`-p 127.0.0.1:3100:3100` 과 `HOST=0.0.0.0` 이 같이 있는 이유.** 서로 다른 두 단계입니다.
> `HOST=0.0.0.0` 은 **컨테이너 안**에서 서버가 열리는 방식이고, 여기서 loopback으로 바꾸면
> 도커가 포트를 전달하지 못해 아예 접속이 안 됩니다. `-p` 앞의 `127.0.0.1:` 은 그 포트를
> **호스트 밖으로** 내보낼 범위이고, 이걸 생략하면 같은 네트워크 전체에 열립니다.
> 안쪽은 열고 바깥은 닫는 게 맞는 조합입니다.
...
위 실행 예시와 `docker-compose.yml`은 포트를 **`127.0.0.1` 에만** 공개합니다. 이게 기본값이고,
그대로 두면 이 항목은 신경 쓸 게 없습니다.

`127.0.0.1:` 을 떼고 `-p 3100:3100` 으로 바꾸면 같은 네트워크 전체에 열리는데, 그러면:

- **`/ws` 에 인증 없이 붙을 수 있습니다.** standalone 모드의 `/ws` 는 Bearer 토큰을 요구하지
  않고(embedded 전용 검사), Origin 헤더가 없는 클라이언트는 Origin 검사도 통과합니다.
  즉 직원 세션 스트림을 그대로 읽을 수 있습니다.
- **폴더 피커의 loopback 검사는 `Host` 헤더에 의존합니다.** 헤더는 호출자가 정하는 값이라
  `curl -H "Host: localhost"` 한 줄로 통과하고, 그러면 파일시스템을 훑을 수 있습니다.

둘 다 "포트가 loopback에만 열려 있다"는 전제 위에 서 있는 설계라, 전제를 깨면 같이 무너집니다.

**리모트 서버에 배포할 때**: 포트는 loopback에 둔 채 SSH 터널이나 VPN으로 접속하세요(아래).
역프록시를 쓴다면 프록시 단에서 인증을 걸고 TLS로 암호화해야 합니다 — 앱은 인증을 하지 않습니다.

### 다른 기기에서 접속하기 — SSH 터널

**앱은 인증을 하지 않습니다.** 로컬 전용이라는 전제로 설계돼 있어서, 접근 통제는 앱 밖에서
해야 합니다. 그래서 포트를 `127.0.0.1` 에 묶어두고 **터널로 접속**하는 것이 지원 방식입니다.
...
브라우저가 `localhost` 로 접속하게 되므로 **코드 변경 없이 전부 정상 작동**합니다. 인증은 SSH가
담당하니 앱 쪽 추가 설정도 없습니다. VPN을 쓰는 경우도 원리는 같습니다.

#### 포트를 직접 여는 방식이 아직 지원되지 않는 이유

`-p 3100:3100` 으로 네트워크에 직접 노출하는 건 **작동은 하지만 인증이 없습니다.** 접속한 쪽이
누구인지 확인하는 절차가 아직 없어서입니다:

- **`/ws` 에 토큰 검사가 없습니다.** Bearer 검사는 embedded(VS Code) 모드에만 걸려 있고
  standalone에는 없습니다(`httpServer.ts`). Origin 검사가 남지만 `isAllowedWsOrigin` 은
  Origin 헤더가 **없으면 통과**시키므로(비브라우저 클라이언트 허용), curl 같은 클라이언트는
  그대로 들어옵니다. 직원 세션 스트림이 열립니다.
- **폴더 피커의 loopback 검사가 `Host` 헤더 기반입니다.** 헤더는 호출자가 정하는 값이라
  `curl -H "Host: localhost"` 로 위조되고, 그러면 파일시스템을 훑을 수 있습니다.

제대로 열려면 `Host` 헤더 대신 **토큰으로** 신원을 확인해야 하는데, 브라우저 WebSocket API는
`Authorization` 헤더를 붙일 수 없어서 쿼리 파라미터나 subprotocol로 토큰을 넘기는 설계가
따로 필요합니다. 아직 없습니다.

`PIXEL_AGENTS_TOKEN` 은 HTTP API 쪽에서만 쓰이며, 위 두 구멍을 막지 못합니다.

#### ⚠️ 요약

**포트를 loopback 밖으로 열지 마세요.** 열면 같은 네트워크의 누구나 인증 없이 직원 세션에
접근하고 파일시스템을 열람할 수 있습니다. 다른 기기에서 쓰려면 SSH 터널이나 VPN을 쓰세요.
...
- **아직 안 되는 것**: 다른 회사 AI(GPT·Gemini) 연결, 자동 업데이트.
  - **외부 기기 원격 접속**: 접속한 쪽이 누구인지 확인하는 절차가 아직 없어서, 포트를 loopback에만
    열어두고 있습니다. **SSH 터널로 접속하면 코드 변경 없이 작동합니다** — 자세히는 위의
    "다른 기기에서 접속하기 — SSH 터널" 절 참고.
