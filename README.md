...
> [!NOTE]
> **2026-07-16 자체 코드 점검 결과 반영 완료.** 점검에서 확인됐던 데이터 유실·크래시급 결함 — 훅 설치가 손상된 `~/.claude/settings.json`을 덮어쓸 수 있던 경로 포함 — 은 **전부 수정되어 main에 반영됐습니다** (4라운드 점검 + 스모크 테스트, 실사용 회귀 없음). 경미한 잔여 항목(동시성 미세 경합 등)은 계속 정리 중이며, 배포용 설치본(GitHub Releases)은 아직 없으므로 사용은 소스 빌드 기준입니다.
