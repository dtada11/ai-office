 ...
 *
 *  Lives in core (not webview-ui) because the team-scaffolding templates
 *  (teamTemplates.ts) resolve a role's title/persona/model from these same
 *  presets on the server side, when it builds the suggested roster for a
 *  newly-created project -- the hire form (webview-ui) needs the exact same
 *  data to do the same thing by hand. One definition, both reuse it. */
